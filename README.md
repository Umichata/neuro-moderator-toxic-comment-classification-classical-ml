...
├── requirements.txt
├── models_cache/              # кэш обученных моделей, если публикуется отдельно
├── feature_cache/             # кэш TF-IDF, SVD и мета-признаков, если публикуется отдельно
└── artifacts/                 # дополнительные артефакты экспериментов
```
